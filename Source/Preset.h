/*
  ==============================================================================
    ...
    Preset.h
    Created: 8 May 2024 4:42:28pm
    Author:  MacJay

    The original MDA JX10 synth included fifty or so preset patches that show off
    the capabilities of this synth. This class adds the factory presets to JX11
    too.
 
    In JUCE terminology the factory presets are called programs. Each program
    is basically a collection of parameter settings.
...
struct Preset {
    Preset(const char* name,
        ...
        float p24, float p25)
    {
//        Copy the name string with a length check
        strncpy(this->name, name, sizeof(this->name) - 1);
        
//        Ensure null-termination.
//        This line ensures that the destination buffer
//        (this->name) is null-terminated.
        this->name[sizeof(this->name) - 1] = '\0';
        
//        Assign parameters to the param array
        param[0] = p0;   // Osc Mix
        param[1] = p1;   // Osc Tune
        param[2] = p2;   // Osc Fine
